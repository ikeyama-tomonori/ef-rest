...
internal static class TypeExtention
{
    ...
    {
        var ignores =
            ignoreAttributes ?? new[] { typeof(JsonIgnoreAttribute), typeof(NotMappedAttribute) };
        ...
        var properties = type.GetProperties()
            .FirstOrDefault(propertyInfo =>
            {
                ...
                if (attributeTypes.Intersect(ignores).Any())
                {
                    return false;
                }
                ...
                var convertedName =
                    jsonSerializerOptions.PropertyNamingPolicy?.ConvertName(propertyInfo.Name)
                    ?? propertyInfo.Name;
